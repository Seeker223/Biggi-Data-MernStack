...
import { ChevronLeft, Trophy, Users, RefreshCw } from "lucide-react";
import { AuthContext } from "../../context/AuthContext";
import { getReferralLeaderboard } from "../../services/api";
...
const ReferralScreen = () => {
  ...
  const loadLeaderboard = async () => {
    try {
      ...
      setLeaderboard(rows);
      setMyCount(Number(payload.myCount || 0));
      ...
      setError(err?.response?.data?.message || "Unable to load referral leaderboard.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const headerSubtitle = useMemo(() => {
    if (monthLabel) return `Top referrers for ${monthLabel}`;
    ...
    <Page>
      ...
      <HeroCard>
        <HeroLeft>
          ...
          <HeroMeta>
            Monthly referrals: <strong>{myCount}</strong>
          </HeroMeta>
          <HeroMeta>Refer more friends to climb the ranks.</HeroMeta>
        </HeroLeft>
        <HeroIconWrap>
          <Trophy size={26} />
        </HeroIconWrap>
      ...
      {loading ? (
        ...
        <EmptyState>
          <Users size={26} />
          <p>No referrals recorded yet this month.</p>
          <span>Share your referral code to appear on the leaderboard.</span>
        </EmptyState>
      ) : (
        <List>
          {leaderboard.map((entry) => (
            <Row key={`${entry.referralCode}-${entry.rank}`}>
              ...
              <CountPill>
                {entry.count} referral{Number(entry.count || 0) === 1 ? "" : "s"}
              </CountPill>
            </Row>
          ))}
        </List>
